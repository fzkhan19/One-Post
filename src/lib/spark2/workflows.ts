...
export function setupTextToImage(
	prompt: string,
...
export function setupTextToVideo(
	...
	// Node 26: FluxGuidance (CFG / text adherence scale)
	if (options?.guidance !== undefined && wf["26"]?.inputs) {
		wf["26"].inputs.guidance = options.guidance;
	}
	// Node 25: RandomNoise seed
	if (wf["25"]?.inputs) {
		wf["25"].inputs.noise_seed = seed;
...
export function setupImageToText(
	imageBuffer: Buffer,
