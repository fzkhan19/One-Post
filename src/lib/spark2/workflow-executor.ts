...
import type { ComfyUIDirectClient } from "./comfyui-client";
import {
	type ComfyUIEventMap,
	GenerationProgress,
	...
	async executeWorkflow(request: WorkflowRequest): Promise<WorkflowResult> {
		...
		if (!this.client.connected) {
			onProgress?.({
				stage: "preparing",
				progress: 5,
				message: "Establishing connection to ComfyUI...",
			});
		...
		if (inputMedia && inputMedia.length > 0) {
			onProgress?.({
				stage: "uploading",
				progress: 10,
				message: `Uploading ${inputMedia.length} assets...`,
			});

			for (const item of inputMedia) {
				if (signal?.aborted)
					throw new Error("Execution aborted during asset upload.");

				console.log(
					`[WorkflowExecutor] Uploading input file: ${item.name} for node ${item.nodeId}`,
				);
				const uploadResult = await this.client.uploadImage(
					item.data,
					item.name,
				);
				...
				if (workflow[item.nodeId]?.inputs) {
					const inputs = workflow[item.nodeId].inputs as Record<
						string,
						unknown
					>;
		...
		if (signal?.aborted)
			throw new Error("Execution aborted prior to prompt submission.");
		onProgress?.({
			stage: "queued",
			progress: 20,
			message: "Submitting prompt to ComfyUI queue...",
		});

		const submission = await this.client.submitPrompt(workflow);
		const promptId = submission.prompt_id;
		console.log(
			`[WorkflowExecutor] Prompt queued successfully. ID: ${promptId}`,
		);
		onProgress?.({
			stage: "queued",
			progress: 25,
		...
		return new Promise<WorkflowResult>((resolve, reject) => {
			let isSettled = false;
			const progressTimer: ReturnType<typeof setTimeout> | null = null;
			...
			const timeoutHandle = setTimeout(() => {
				fail(
					new Error(`Workflow execution timed out after ${timeout / 1000}s`),
				);
			}, timeout);
			...
			const onWsExecuting = (data: ComfyUIEventMap["executing"]) => {
				if (data.prompt_id !== promptId) return;
				if (data.node === null) {
					// Node is null means finished executing all nodes on ComfyUI
					finishExecution();
					return;
				}
				onProgress?.({
			...
			const onWsExecuted = (data: ComfyUIEventMap["executed"]) => {
				...
				if (String(data.node) === String(outputNodeId)) {
					finishExecution();
				}
			};
			...
			const finishExecution = async () => {
				if (finishedHandling || isSettled) return;
				...
				try {
					onProgress?.({
						stage: "downloading",
						progress: 90,
						message: "Downloading outputs from ComfyUI...",
						promptId,
					});
					console.log(
						`[WorkflowExecutor] Fetching history for prompt: ${promptId}`,
					);

					const history = await this.client.getHistory(promptId);
					if (!history) {
						throw new Error(
							`Execution succeeded but history for ${promptId} could not be retrieved.`,
						);
					}
					...
					if (!targetOutputs) {
						console.warn(
							`[WorkflowExecutor] Output node ${outputNodeId} not found in ComfyUI history. Checking all output nodes...`,
						);
					}
					...
					const nodesToScan = targetOutputs
						? [outputNodeId]
						: Object.keys(outputs);
					...
					for (const nodeKey of nodesToScan) {
						...
						if (nodeOutput.images && Array.isArray(nodeOutput.images)) {
							for (const img of nodeOutput.images) {
								...
								if (!res.ok)
									throw new Error(
										`Failed to download output image ${img.filename}`,
									);
								resolvedFiles.push({
						...
						if (nodeOutput.gifs && Array.isArray(nodeOutput.gifs)) {
							for (const gif of nodeOutput.gifs) {
								...
								if (!res.ok)
									throw new Error(
										`Failed to download output video/gif ${gif.filename}`,
									);
								resolvedFiles.push({
									buffer: Buffer.from(await res.arrayBuffer()),
									filename: gif.filename,
					...
					if (resolvedFiles.length === 0 && !rawText) {
						throw new Error(
							"ComfyUI finished execution but yielded no compatible image, video, or text outputs.",
						);
					}
			...
			const onWsSuccess = (data: ComfyUIEventMap["execution_success"]) => {
				if (data.prompt_id !== promptId) return;
				finishExecution();
			};
			...
			const onWsError = (data: ComfyUIEventMap["execution_error"]) => {
				...
				fail(
					new Error(`ComfyUI Server Error: ${errMsg} (Node ${data.node_id})`),
				);
			};
			...
			// Robust fallback: poll history every 5 seconds in case WebSocket messages were dropped
			pollInterval = setInterval(async () => {
				if (isSettled || finishedHandling) return;
				try {
					const hist = await this.client.getHistory(promptId);
					if (hist?.status?.completed) {
						console.log(
							`[WorkflowExecutor] Prompt ${promptId} completed detected via history poll.`,
						);
						finishExecution();
					}
				} catch (_) {}
